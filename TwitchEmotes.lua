...
Emoticons_Settings={
  ...
  local origsettings = {
    ...
    for index,folder in pairs(addon.keyset) do
      table.insert(dropdown_options, n);
      dropdown_options[n] = {folder};
      for key, path in pairs(addon.defaultpack[folder]) do
        table.insert(dropdown_options[n], key);
      end
      n=n+1;
    end
  
   -- for k,v in pairs(dropdown_options[0]) do
    --    print(k, v);
   -- end
  ...
  function Emoticons_LoadChatFrameDropdown(self, level, menuList)
	...
	if (level or 1) == 1 then
	  ...
	  for ke,va in ipairs(dropdown_options[menuList]) do
        folder = addon.keyset[menuList];
        -- print("key", ke,"va", va, "menuList", menuList, "folder", folder);
		if (first) then
		  first = false;
		else
		  --print(ke.." "..va);
		  info.text       = "|T"..addon.defaultpack[folder][va].."|t "..va;
		  info.value      = va;
  ...
  function Emoticons_Deformat(msg)
    for index,folder in pairs(addon.keyset) do
        for k,v in pairs(addon.defaultpack[folder]) do
            msg=string.gsub(msg,"|T"..addon.defaultpack[folder][v].."%:28%:28|t",k);
        end
    end
        return msg;
  end
    ...
    for index,folder in pairs(addon.keyset) do
        for k,v in pairs(addon.defaultpack[folder]) do
          if (string.find(msg,k,1,true)) then
            msg = string.gsub(msg,"(%s)"..k.."(%s)","%1|T"..addon.defaultpack[folder][k].."|t%2");
            msg = string.gsub(msg,"(%s)"..k.."$","%1|T"..addon.defaultpack[folder][k].."|t");
            msg = string.gsub(msg,"^"..k.."(%s)","|T"..addon.defaultpack[folder][k].."|t%1");
            msg = string.gsub(msg,"^"..k.."$","|T"..addon.defaultpack[folder][k].."|t");
            msg = string.gsub(msg,"(%s)"..k.."(%c)","%1|T"..addon.defaultpack[folder][k].."|t%2");
            msg = string.gsub(msg,"(%s)"..k.."(%s)","%1|T"..addon.defaultpack[folder][k].."|t%2");
          end
        end
    end	
